...
\subsection{Kriterien für die Spielidee}\label{ssec:kriterien}
	...
	\begin{enumerate}
		...
		\item{In anfänglichen Gesprächen wurde sich darauf geeinigt, dass das Spiel in keinem Fall ein Rollenspiel sein soll.}
...
\subsection{Brainstorming \& Game Design}\label{ssec:idee}
	...
	Die Ideen wurden anhand der Kriterien aus Kapitel \ref{ssec:kriterien}, aber auch nicht eindeutigen subjektiven Kriterien wie \enquote{Eigeninteresse an der Idee}, \enquote{Lust die Idee umzusetzen} und weiteren welche nicht klar Formulierbar sind, abgewogen.
	\begin{enumerate}
...
\subsection{Funktionale und nicht funktionale Anforderungen}\label{ssec:requirements}
	...
	\begin{description}
		...
		\item[Level-Gruppe wechseln]{
			Der Spieler kann die Level-Gruppen rotieren, jede besitzt zwei Level als Qualifikationsrennen (normale Rennkarten) und einen Level für welchen ein gewisser Preis an gelösten Aufgaben gezahlt werden muss (Pokalrennen).
			Jede Level-Gruppe ist in einem gewissen Stil durch thematisiert wobei die Karten für die Pokalrennen etwas außergewöhnlicher sein sollen.
		}
		...
		\item[Onlinespiel starten]{ Ebenso wie die Optionen ist dies ein speziell gekennzeichneter \enquote{Level} welcher es ermöglicht mit Zufälligen anderen Spielern ein Rennen zu fahren. }
		\item[Extra Aufgaben Lösen]{ Ein Icon am Rand des Bildschirms, welches es ermöglicht Punkte durch Aufgabenlösen zu sammeln. }
...
\subsection{Grundsätzliche Eigenschaften des Spiels}
	...
	In den Bereich der Spielmechanik fallen all jene Regeln und Eigenschaften, welche das Spielgeschehen (Die Spielerfahrung des Spielers) beeinflussen und diese ausmachen.
		...
		Für den Umfang des Spiels wurde eine Größe von drei Rennen pro Level-Paket festgelegt. Der Abschluss eines einzelnen Qualifikaionsrennens für die Freischaltung des Pokalrennens ist zwar grundsätzlich möglich, führt jedoch zu einer Minderung des Spielumfangs. Um dies zu vermeiden, besteht zumindest die Anforderung, beide Qualifikationsrennen abschließen zu müssen. Die Reihenfolge der Qualifikationsrennen spielt im Bereich der Entwicklung keine Rolle. Dieser Aspekt ist insofern zu vernachlässigen. Für den Spieler könnte eine sofortige Verfügbarkeit beider Qualifikationsrennen ein gewisses \enquote{Freiheitsgefühl} auslösen. Aus diesem Grund werden mit Beginn eines Level-Pakets beide Qualifikationsrennen sofort verfügbar.
		...
		Das richtige Beantworten von Aufgaben sollte ebenfalls in mit die Gewinnbedingungen fliesen, wenn keine einzige Aufgabe korrekt war verliert der Spieler. Die konkrete Umsetzung davon befindet sich in Tabelle \ref{pokal-tabelle}.
		...
		Als Belohnung und Bestrafung wird eine Sammlung an Reaktionstönen fungieren. Beispielsweise ein Jubeln oder Händeklatschen als positive Rückmeldung und Ausbuhen als negative Rückmeldung.
	...
	Im Bereich der sonstigen Eigenschaften sollen Eigenschaften betrachtet werden, die nicht direkt die Spielmechanik betreffen. Dazu gehören unter Anderem Kameraeinstellungen, das Optionsmenü und ähnliches.
		...
		\begin{description}
			...
			\item[Lenkrad auf dem Touchscreen]{ Auf dem Bildschirm wird ein Lenkrad aus der Sicht des Fahrers gezeigt, der Nutzer kann dieses mittels tappen und ziehen drehen und ein links oder Rechtsfahren bewirken. }
			\item[Direkteingabe über den Bildschirm]{ Mit dem Finger auf den Bildschirm tappen oder halten, das Spiel berechnet dann die gewünschte Richtung und lässt das Auto an die zuletzt berührte Stelle fahren. In manchen Implementierungen werden bei dieser Art der Steuerung auch Hindernisse umgangen,\footnote{Beispielsweise in \enquote{League of Legends}.} dies ist in Rennspielen nicht gut, weil hier die Schwierigkeit im Fahren an sich besteht.  }
			\item[Digitalem Joystick]{ Joystick welcher relativ zum Auto interpretiert wird. Nach rechts ziehen lässt das Auto eine Rechtskurve fahren. Nach unten ziehen bremst und kann auch rückwärts fahren bewirken. }
			\item[Digitalem Joystick nur Richtung]{ Knüppel, welcher mit zwei Achsen die Fahrtrichtung direkt angibt. Die Richtung sollte relativ zur Kamera interpretiert werden, dass nach rechts ziehen auch einem nach rechts Fahren entspricht. }
		\end{description}
		...
		Ein Loslassen des Gaspedals lässt das Fahrzeug bis zum stillstand rollen. Ein Betätigen des Bremspedals führt zu einer deutlich schnelleren Verringerung der Geschwindigkeit.
		...
		\paragraph{Heads-Up Display:}
			...
			Bei den verschiedenen Elementen soll hier farblich klar sein mit welchen interagiert werden kann und welchen nicht.

		\paragraph{Kameraperspektive:}
		Grundsätzlich sind für Rennspiele drei Kameraperspektiven denkbar. Diese sind:
		\begin{itemize}
			\item{ Kamera zeigt das innere des Fahrzeugs und schaut durch die Frontscheibe. Dies wird auch \enquote{First-Person} genannt. }
		...
		Um den Umfang des Spiels zu vervollständigen, muss an dieser Stelle noch die Musik und Sound im Allgemeinen betrachtet werden. Im Brainstorming wurde entschieden, dass das Spiel eine Hintergrundmusik erhalten soll. Zudem werden Töne für: Kollisionen, die Fahrzeuge und bestimmte Momente im Spiel: Start, Ziel und Lösen einer Aufgabe benötigt.
