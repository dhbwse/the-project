...
\subsection{Brainstorming \& Game Design}\label{ssec:idee}
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\item{Fehler, die dem Spieler in den Experimenten passieren, führen zum Scheitern des Versuchs.}
	...
	\begin{enumerate}
		...
		\begin{itemize}
			\item{Ein rollender Ball und ein rollender Würfel (der logischerweise ungleichmäßige Bewegungen vollführt).}
			\item{Ein rennender Mensch oder ein rennendes Tier (die die gleiche Geschwindigkeit wie Autos erreichen).}
			\item{Ein Auto oder ein Flugzeug, das sich in Bodennähe aufhält und auf die Gegebenheiten der Strecke ebenso reagieren muss.}
		\end{itemize}
		...
		\item{Im Mehrspieler-Modus müssen die Spieler Aufgaben absolvieren, um das Rennen beginnen zu können.}
			\begin{itemize}
				...
				\item{Können die Aufgaben nicht gelöst werden, darf der Spieler nach einer Zeitverzögerung dennoch losfahren. Spieler, die die Aufgaben gelöst haben, haben im Rennen dadurch einen Vorteil.}
...
\subsection{Funktionale und nicht funktionale Anforderungen}\label{ssec:requirements}
	...
	\begin{description}
		\item[Spieler]{ Die Person, die das Spiel auf dem Handy installiert hat und spielt. }
		\item[Aufgabensteller]{ Die Person, die neue Aufgaben für das Spiel zusammenstellt, bestehende editiert oder auch Aufgaben entfernt. Die Person benötigt eine Methode die Aufgaben zu aktualisieren. }
	\end{description}
	...
	\begin{description}
		...
		\item[Level-Pakete wechseln]{
			Der Spieler kann die Level-Pakete rotieren, jedes besitzt zwei Level als Qualifikationsrennen (normale Rennkarten) und einen Level, für den eine Teilnahmegebühr anfällt (Pokalrennen).
		...
		\item[Level wählen]{ Der Spieler fährt mit dem Auto auf einen Level, darauf erscheint ein Auswahlbildschirm für folgende zwei Optionen. Falls der Level ein Pokalrennen ist, muss der Spieler eine Teilnahmegebühr zahlen.
			\begin{description}
				\item[als Rennen gegen die Zeit starten]{ In diesem Spielmodus ist eine Zeit vorgegeben, innerhalb welcher der Spieler den Level bestehen soll. Ebenso sind zwei kleinere Zeiten vorgegeben, die den Spieler herausfordern, sich mehr anzustrengen. }
		...
		\item[Extra Aufgaben Lösen]{ Ein Icon am Rand des Bildschirms, das ermöglicht, Münzen durch Lösen von Aufgaben zu sammeln. }
		\item[Einkaufen]{ Wie bereits im Rahmen der Ingame-Währung erwähnt, kann der Spieler beispielsweise Fahrzeuge und Farben erwerben.
			\begin{description}
				\item[Gegenstand Kaufen]{ Der Spieler erhält im Austausch für Ingame-Währung den Artikel, dieser wird dann auch ausgewählt. Im Ingame-Shop existieren Gegenstände, die verbraucht werden, zum Beispiel Zeitboni und Gegenstände, die dem Spieler verfügbar bleiben, wie Fahrzeuge. }
	...
	\end{description}
	Alle Funktionalitäten lassen sich in einem Use-Case Diagramm abbilden, das die Autoren bei der Entwicklung durch vereinfachte und übersichtliche Koordination und Aufgabenaufteilung unterstützt.
...
\subsection{Eigenschaften des Spiels}\label{sssec:spielmechanik}
	...
	\subsubsection*{Steuerung:}
		...
		\begin{description}
			...
			\item[Digitaler Joystick]{ Ein Joystick, der relativ zum Auto interpretiert wird. Nach rechts ziehen lässt das Auto eine Rechtskurve fahren. Nach unten ziehen bremst und kann auch rückwärtsfahren bewirken. }
			\item[Digitaler Joystick (nur Richtung)]{ Ein Joystick, der mit zwei Achsen die Fahrtrichtung direkt angibt. Die Richtung sollte relativ zur Kamera interpretiert werden, sodass ein nach rechts ziehen des Joysticks auch einem nach rechts fahren entspricht. }
		\end{description}
		...
		Aus diesen Gründen wird eine Steuerung mit digitalem Joystick, der die Fahrtrichtung übernimmt und Pedalen für Gas und Bremse gewählt. Rückwärts fahren soll dabei nur aus dem Stillstand durch ein Lenken nach hinten und Gas geben möglich sein.
		...
		Ein Beispiel für digitale Joysticks, der in Mobilen-Spielen relativ üblich sind, ist in der folgenden Abbildung zu sehen.
	...
	\subsubsection*{Heads-Up Display:}\label{HUD}
			...
			Hinzu kommen die angezeigte Aufgabe und die beiden Antwortmöglichkeiten, eine Übersichtskarte, die den Streckenverlauf anzeigt, und eine Zeitanzeige, die den zeitlichen Rahmen klar macht.
