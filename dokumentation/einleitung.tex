...
\section{Einleitung}
	Üblicherweise geht es in der Spieleentwicklung darum dem Spieler eine bestimmte Emotion zu vermitteln, meistens ist dies Spaß und Freude\footcite{persona} Soziologen und Lehrer <TODO:Quelle? und genauer> hatten die Idee dass durch Spiele auch Wissen vermittelt werden kann. Wenn diese Lernspiele <TODO:Beispiele> betrachtet werden fällt oft auf dass der Spielspaß dabei vernachlässigt wurde.
...
\subsection{Vorgehen}
	Für die Zielgruppe des Spiels wird ein Play-Persona\footcite{persona} ausgearbeitet, danach ein Gamification-Modell auf dieses Persona zugeschnitten. Das Gamification-Modell soll Spieler anregen das Spiel länger zu spielen und so mehr zu lernen. Der Fokus liegt dabei stark auf der Verbindung von Gamification und spielerischem Lernen.
	Das Spiel soll in einer gängigen Game-Engine entwickelt werden. Um eine passende Game-Engine zu wählen eine Evaluation durchgeführt (siehe \ref{ssec:engineeval}).
	Die Entwicklung des Spiels geschieht über anfängliches Prototyping und eine anschließenden Implementierung in der Gameengine. Am Ende der Implementierung soll das entstandene Spiel in die bereits vorhandene Lernplattform von Prof. PhD. Kay Berkling eingegliedert und über diese vertrieben werden.
